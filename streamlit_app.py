...
# Get saved phase changes if available.
saved_phase_changes = st.session_state.get("phase_changes", ["Temperature", "Volume/Pressure", "Addition"])
phase_change1 = st.selectbox("Boundary 1 Change", ["Temperature", "Volume/Pressure", "Addition"],
                             index=["Temperature", "Volume/Pressure", "Addition"].index(saved_phase_changes[0]),
                             key="phase1")
phase_change2 = st.selectbox("Boundary 2 Change", ["Temperature", "Volume/Pressure", "Addition"],
                             index=["Temperature", "Volume/Pressure", "Addition"].index(saved_phase_changes[1]),
                             key="phase2")
phase_change3 = st.selectbox("Boundary 3 Change", ["Temperature", "Volume/Pressure", "Addition"],
...
if "Temperature" in phase_changes:
    temp_effect = st.slider("Temperature Effect (for Temperature changes)",
...
if "Volume/Pressure" in phase_changes:
    vol_effect = st.slider("Volume/Pressure Effect",
...
st.info("Configuration saved! Now you can navigate to the Simulation page via the sidebar.")
